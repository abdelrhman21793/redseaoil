<div>
    <form class="row g-3" method="POST" action="{{ route('troubleshootstructures.store') }}" style="width: 1200px;">
        @csrf
        ...
        <select class="col-md-6" name="option_id" class="form-control" wire:model="selectedOption">
            @foreach ($troubleshoots as $troubleshoot)
                <option value="{{ $troubleshoot->id }}"{{ $troubleshoot->id == $selectedTroubleshoot ? 'selected' : '' }}>{{ $troubleshoot->name }}</option>
            @endforeach
        </select>
            ...
            <div class="col-md-6" style="margin-bottom: 35px">
                ...
                @foreach ($structuresDes as $index => $stuctureDesc)
                    <div class="input-group mb-3" style="margin-left: 10px; display: flex; align-items: center;">
                        ...
                        <x-forms.input type="text" class="form-control" style="flex: 1;"
                        ...
                        <a href="#" class="btn btn-outline-danger" style="padding: 12px; margin-left: 10px;"
