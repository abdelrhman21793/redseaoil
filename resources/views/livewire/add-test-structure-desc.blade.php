    ...
    <form class="row g-3" method="POST" action="{{ route('teststructures.store') }}" style="width: 1200px;">
        ...
        <select class="col-md-6" name="option_id" class="form-control" wire:model="selectedOption">
            @foreach ($tests as $test)
                <option value="{{ $test->id }}"{{ $test->id == $selectedTest ? 'selected' : '' }}>{{ $test->name }}</option>
            @endforeach
        </select>
                ...
                @foreach ($structuresDes as $index => $stuctureDesc)
                    <div class="input-group mb-3" style="margin-left: 10px; display: flex; align-items: center;">
                        ...
                        <a href="#" class="btn btn-outline-danger"
                            wire:click.prevent="removeStruDesc({{ $index }})" style="padding: 12px; margin-left: 10px;">Remove Desc</a>
                    </div>
