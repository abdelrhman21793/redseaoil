<div>
    <x-alert />
    <form class="row g-3" method="POST" action="{{ route('surveystructuresdesc.update', $structure->id) }}"
        style="width: 1400px;">
        @csrf
        @method('put')
        ...
        <select class="col-md-6" name="option_id" class="form-control" wire:model="selectedOption">
            @foreach ($options as $option)
                <option value="{{ $option->id }}" {{ $option->id == $structure->survey_id ? 'selected' : '' }}>
                    {{ $option->name }}</option>
            @endforeach
        </select>
        <div class="card-body mx-auto">
            <div class="col-md-6" style="margin-bottom: 35px">
                ...
                @foreach ($structure->survey_structure_descriptions as $index => $struct_desc)
                    ...
                    <div class="input-group mb-3" style="margin-left: 10px; display: flex; align-items: center;">
                        ...
                        <a href="{{ route('surveydesc.delete',$struct_desc->id) }}" class="btn btn-outline-danger"
