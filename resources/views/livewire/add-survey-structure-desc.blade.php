<div>
    <form class="row g-3" method="POST" action="{{ route('surveystructures.store') }}" style="width: 1200px;">
        @csrf

        <label>Option Name</label>
        <select class="col-md-6" name="option_id" class="form-control" wire:model="selectedOption">
            @foreach ($surveys as $survey)
                <option value="{{ $survey->id }}"{{ $survey->id == $selectedSurvey ? 'selected' : '' }}>{{ $survey->name }}</option>
            @endforeach
        </select>
                ...
                @foreach ($structuresDes as $index => $stuctureDesc)
                    <div class="input-group mb-3" style="margin-left: 10px; display: flex; align-items: center;">
                        ...
                        <x-forms.input
                            ...
                            wire='wire:model="structuresDes.{{ $index }}.input"'
                            placeholder="Input"
                            style="flex: 1;"
                        />
