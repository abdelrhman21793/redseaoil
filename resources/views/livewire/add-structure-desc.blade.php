<x-alert />
<div>
    <form class="row g-3" method="POST" action="{{ route('structures.store') }}" style="width: 1200px;">
        @csrf
                ...
                @foreach ($structuresDes as $index => $stuctureDesc)
                    <div class="input-group mb-3" style="margin-left: 10px; display: flex; align-items: center;">
                        <x-forms.input
                            ...
                            wire:model="structuresDes.{{ $index }}.input"
                            placeholder="Input"
                            style="flex: 1;"
                        />

                        <select
                            name="structuresDes[{{ $index }}][type]"
                            wire:model="structuresDes.{{ $index }}.type"
                            class="form-control"
                        ...
                        <a href="#"
                            ...
                            style="padding: 12px; margin-left: 10px;"
                        >
                            Remove Desc
                        </a>
                    </div>
                                ...
                                @foreach ($structureDesMenu['data'] as $dataIndex => $data)
                                    <div class="form-group" style="margin: 5px;">
                                        <input type="text"
                                            ...
                                            placeholder="Data" />
                                        <a href=""
                                            wire:click.prevent="removeData({{ $index }}, {{ $dataIndex }})">
                                            <span class="material-symbols-outlined">delete</span>
                                        </a>
                                    </div>
                                @endforeach

                                <button class="btn btn-outline-secondary" wire:click.prevent="addData({{ $index }})">Add Data</button>
                                <button class="btn btn-outline-danger" wire:click.prevent="removeStruMenu({{ $index }})">Remove Menu</button>
                            </div>
                        </div>
                    </div>
                    <hr style="margin-bottom: 10px">
