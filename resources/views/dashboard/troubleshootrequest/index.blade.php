            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <table class="table">
                    <thead>
                      <tr>
                        <th scope="col">#</th>
                        <th scope="col">Troubleshoot Well</th>
                    ...
                    <tbody>
                        @foreach ($requests as $request)
                            <tr>
                                ...
                                <td>{{ $request->troubleshoot_well->name }}</td>
                                <td>{{ $request->description }}</td>
                                <td>{{ $request->user->name }}</td>
                                <td class="btn-group">
                                    <a href="{{ route('troubleshootrequests.edit',$request->id) }}"
                                        class="btn btn-sm btn-outline-success">Accept</a>
                                    <form method="POST" action="{{ route('troubleshootrequests.reject',$request->id) }}">
                                        @csrf
