                    ...
                    <tbody>
                        @foreach ($requests as $request)
                            <tr>
                                ...
                                <td>{{ $request->test_well->name }}</td>
                                <td>{{ $request->description }}</td>
                                <td>{{ $request->user->name }}</td>
                                <td class="btn-group">
                                    <a href="{{ route('testrequests.edit',$request->id) }}"
                                        class="btn btn-sm btn-outline-success">Accept</a>
                                    <form method="POST" action="{{ route('testrequests.reject',$request->id) }}">
                                        @csrf
