...
class EditTestStructure extends Component
{
    ...
    public function mount($structure)
    {
        ...
        $this->options=Test::all();
        $this->structuresDesUpdate=[];
        $this->structuresDes = [];
        $this->structuresDesMenu = [];
        $this->types = ['String', 'Int', 'Boolean', 'Date', 'MultiText','List','User'];
    }
    public function render()
    {
        return view('livewire.edit-test-structure');
    }
    public function addStruDesc()
    {
        $this->structuresDes[] = ['input', 'type', 'is_require','view'];
    }
