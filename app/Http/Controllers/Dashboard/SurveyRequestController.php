...
class SurveyRequestController extends Controller
{
    ...
    {
        $requests=SurveyRequest::where('status','pending')->with(['survey_well.well','user'])->paginate(5);
        return view('dashboard.surveyrequest.index',compact('requests'));
    }
    ...
    {
        $requestEdit= SurveyRequest::with(['user','survey_well'])->findOrfail($id);
